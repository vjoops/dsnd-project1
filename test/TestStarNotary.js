...
it('can add the star name and star symbol properly', async() => {
    ...
    let instance = await StarNotary.deployed();
    let expectedTokenName = 'VJCoin';
    let expectedSymbol = 'VC';
    let user1 = accounts[1];
    let starId1 = 6;
    await instance.createStar('awesome star', starId1, {from: user1});
    let tokenName = await instance.tokenName();
    let symbol = await instance.symbol();
    assert.equal(tokenName, expectedTokenName);
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let user1 = accounts[1];
    let starId = 10;
    let expectedStarName = 'my new star';
    await instance.createStar(expectedStarName, starId, {from: user1});
